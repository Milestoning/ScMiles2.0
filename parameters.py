...
class parameters:
    ...
    def __init__(self, MS_list=None, Finished=None, MS_new=None, 
                 ...
                 min_grid_value=None, starting_iteration=None, initial_anchors=None) -> None:
        ...
        self.jobsubmit = 'qsub'    # command for job submission
        self.jobcheck = 'qstat'    # command for job check 
        self.nodes = []  # available node list. Enter the names of the nodes in a file my_project_input/nodelist
        self.initial_traj = 10    # number of trajs start from each anchor (used in seek step)
        self.initialTime = 50     # time step in ps for initial trajs (used in seek step)
        self.MS_list = set()    # milestone list
        self.ignorNewMS = False    # ignore new milestones found by free traj
        #self.Finished = set()   # milestones that finished free trajs #no longer used
        #self.finished_constain = set()  # milestones that finished sampling #no longer used
        ...
        self.trajPerLaunch = 100    # number of free trajs to launch each time
        ...
        self.outputname = 'ScMilesProject'    # output name
        ...
        self.flux = []      #flux
        self.colvarNumber = colvarNumber #Number of colvars used
        self.seek_restartfreq = 1000 #restartfreq used in seek. Only really used if different than in free step
        self.anchors = anchors #list of anchor values from anchor.txt
        self.max_jobs = 9999999 #Max amount of jobs to be submitted at once
        #self.split_jobs = False 
        #These filepaths are just to simplify so we don't have to find them over and over again
        self.ScMilesPath = os.path.dirname(os.path.abspath(__file__))  #path
        self.parentDirectory = os.path.abspath(os.path.join(self.ScMilesPath, os.pardir)) #path/ScMiles
        self.crdPath = os.path.join(self.parentDirectory, 'crd') #path/crd
        self.seekPath = os.path.join(self.crdPath, 'seek') #path/crd/seek
        self.inputPath = os.path.join(self.parentDirectory, 'my_project_input') #path/my_project_input
        self.outputPath = os.path.join(self.parentDirectory, 'my_project_output') #path/my_project_output
        self.currentPath = os.path.join(self.outputPath, 'current') #path/my_project_output/current
        self.AnchorPath = os.path.join(self.inputPath, 'anchors.txt') # file path for anchor #path/my_project_input/anchors.txt
        self.restart = False #Whether or not we are restarting the process
        self.correctParameters = True #Returns False and terminates simulation if parameters are not correct.
        self.traj_per_script = [1,1,1] #seek, sample, and free
        ...
        self.not_finish_trajs = None #Trajectories added if they need to be rerun after restarting the simulation
        ...
        #self.grid_dict = None 
        self.pbc_names = [] #The name of periodic variables
        self.l = None #L values used with periodicity, should be the same length as pdb_names 
        self.substitution = None #If we are substituting one colvar with other variables. Like if p is made up of v1,v2,...,v5
        self.origin = 1 #Used in namd_custom_conf
        self.max_lifetime = None #If a lifetime is greater than this
        self.velocity_weighting = False
        self.scale_rmsd = False #if we want to scale our colvars that are used. Entered as a list
        self.compute_only = False #No longer needed since further_analysis was added
        self.targetNumSteps = 50000 #Used if we are using pdb files for sampling to prevent errors
        self.pdb_sampling = False #Means we are using pdb files for sampling
        self.restartfreq = None #If our seek restart freqency is different from our free restart frequency
        self.ignore_transitions = None #Used in further_analysis, used if you want to ignore certain transitions
        self.analysis_ignore_milestones = None #Used in further_analysis, used if you want to ignore certain milestones
        self.k_min_sum = None #If the sum of a row or column is less than this value, that mielstone gets deleted
        self.k_cutoff = 0
        self.min_grid_value = None #If you are using grid and have a min value. Like if your delta is 30, smallest anchor is -160, but only want the grid to go to -180
        self.max_grid_value = None #Same as min, but max
        self.starting_iteration = 1 #Only used in further_analysis if we are doing a single iteration rather than all iterations
        self.initial_anchors = None
    ...
    def initialize(self):
        ...
        parameter_list = (('method', 'method', 'integer'),
                        ...
                        ('initial_anchors','initial_anchors','integer'))
        ...
        if os.path.isfile(os.path.join(self.inputPath, 'nodelist')):
            with open(file=self.inputPath + '/' + 'nodelist') as f:
                for line in f:
                    if "#" in line:
                        continue
                    line = line.split("\n")
        ...
        if self.initial_anchors == None:
            self.initial_anchors = self.AnchorNum        
