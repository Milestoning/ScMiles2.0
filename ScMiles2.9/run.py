...
class launch:
    def __init__(self, parameter, step, loop1=None, loop2=None, colvar_file=None,
        ...
        if self.parameter.software == 'namd': 
            self.collective_variables = 'colvar' #namd uses colvar instead of plumed
            if self.step == 'seek' or self.step == 'free': #Seek and sample use the same filenames
            ...
            self.collective_variables = 'plumed' #gromacs uses plumed
            ...
            self.collective_variables = 'plumed' #Lammps is unifished, but uses plumed when it is finished.
            if self.step == 'seek' or self.step == 'free':
                self.colvar_file = 'plumed_free.dat'
        ...
        if self.step == 'free':
            self.time = 200 #The time in between checking trajectories
            self.script_name = 'MILES' #the name of our submit script when submitted to pbs or slurm
            self.loop1 = self.parameter.MS_list.copy() #This is our 'outer loop' when we are looping through everything
            self.loop2 = range(1, self.parameter.trajPerLaunch + 1) #'inner loop'. So we go through our milestones, then each traj for each milestone
            self.traj_per_script = self.parameter.traj_per_script[2] #This is the traj_per_script entry that is relevant for our step we are on
            ...
            self.time = 60
            self.script_name = 'a'
            self.loop1 = range(1, self.parameter.initial_anchors + 1)
        ...
        self.start_frame = {} #This is just where we are starting from. We use a function to populate it. The main purpose of this is for restarting the script
    ...
    def launch_trajectories(self, possible_ms=None, lastLog=None):
        ...
        if lastLog != None:
            if self.step == 'sample':
                ...
                self.resubmit = True #This means we need to look backwards and see if the last group of trajectories launched successfully or if any need to be submitted again
                lastLog = None #We no longer need to use our lastLog so it is just set to none
        print_log = self.__prepare_trajectory_files() #This step is where we make our namd/gromacs file, our plumed/colvar file, and our submit file
        if print_log == True: #If we did create files, this returns True and is logged here
            log("All files prepared for {} step".format(self.step))      
        print_log = self.__submit_trajectories() #This looks for files called submit (not submit_done) and submits the jobs. The submit files are then changed to
                                                 #the name submit_done
        if print_log == True: #Again, if we did submit jobs, then this is logged and we check the jobs every self.time amount of seconds.
            log("All trajectories launched")  
            self.__check_progress()
            log("Trajectories finished running")
        if self.step != 'sample': #If we are doing seek or free
            count = self.__post_launch(possible_ms) #post launch finds our ending milestones and creates the necessary files
            log("Post launch analysis of trajectories completed")
            return count
        elif self.step == 'sample': #If we are using sample, we do not need to do post launch, we just move files to the restart folder
                                    #If we are using gromacs, we don't actually need the restart folder for anything, it is just more of a placeholder
            self.__error_check() 
            log('Restarts moved to restart folders')
            self.move_files() #Makes the restart folder and moves files appropriately
        log("Finished launching {} trajectories. Moving to next step".format(self.step)) #Always logged when we are done with sampling
        self.parameter.restart = False #At this point it doesn't matter if we restart or not, so it gets turned to False
    ...
    def __start_frames(self):
        ...
        '''
        for i in self.loop1:
            path = self.find_parent_path(i) #this just finds our path like this: if 1=MS1_2, our parent path is crd/1_2
            next_frame = get_next_frame_num(path) #finds the next frame. So if we already have 100 trajectories in crd/1_2, this would return 101
            if self.step == 'sample': 
    ...
    def velocity_weighting(self):
        #as of 5/27, this is incomplete, I just started it AH
        for i in self.loop1:
            count = 0
            tmp=[]
            for j in self.loop2:
    ...
    def __prepare_trajectory_files(self):
        ...
        '''
        #Velocity weighting is unfinished so this will always go to none (as of now)
        ...
        for i in self.loop1:
            ...
            for j in self.loop2:
                trajectory_number = 0
                [anchor1, anchor2] = get_anchors(i) #This just takes our milestone string, like MS1_2, and extracts the anchors to a list, like [1,2]
                count += 1
                submit = False
                if self.step != 'sample': #sample does not have trajectory numbers since there is only one done per milestone
                    trajectory_number = self.start_frame[i] + j - 1
                #print(self.start_frame)
                path = self.find_parent_path(i, trajectory_number,velocity_traj=velocity_traj) #Gives up our 
                ...
                if self.step == 'free':
                    ...
                    frame = None
                if self.parameter.milestone_search == 2 and self.step != 'seek':
                    if self.parameter.all_grid[str(anchor1) + '_' + str(anchor2)][2] == 'c':
                        continue
    ...
    def __check_finished(self, path):
        ...
        '''
        if self.parameter.software == 'namd':
            if os.path.isfile(path + '/stop.colvars.state'): #if a stop.colvars.state exists, it means our trajectory has run and finished successfully
                return True
            elif os.path.isfile(path + '/1.log'): #if we don't have stop.colvars.state but do have a log file, we know the trajectory was submitted but don't know
                                                  #if it was finished or not
                with open(path + '/1.log') as f:
                    for line in f:
                        if 'End of program' in line: #End of program means that it did finish, it probably just did not hit a milestone
                            return True
    ...
    def __script_gromacs(self, newScriptPath, name, script, a1, snapshot):
        ...
        if self.step == 'seek': #if we are seeking, our input information is coming from the original input files
            ...
            if os.path.exists(self.parameter.seekPath):
                gro_option = ' -t'
                config = ' -c ' + relative_input + '/conf.gro'
            else:
                gro_option = ' -c'
        ...
        with FileInput(files=newScriptPath + '/submit', inplace=True) as f:
            for line in f:
                ...
                if 'source' in info and source_found == False:
                    source_found = True
...
# Alfredo: I comment these 2 lines:
...
#                 info = line.split("#")[0].split()
        ...
        if self.parameter.namd_conf == True:
            #if self.step != 'free' and self.parameter.milestone_search in (0,1,2):    
            ...
            for line in f:
                ...
                if "coordinates" in line and "bincoordinates" not in line.lower():
                    ...
                    if self.step == 'sample' and os.path.exists(MSpath + '/seek.ms.pdb'):
                        info[1] = "./seek.ms.pdb"
                    elif self.step == 'sample' and not os.path.exists(self.parameter.inputPath + '/pdb/' + str(a1) + '.pdb') and not self.parameter.pdb_sampling:
                        info[1] = self.parameter.inputPath + '/pdb/1.pdb'
                    elif self.step == 'free' and not os.path.exists(self.parameter.inputPath + '/pdb' + str(a1) + '.pdb'):
                        info[1] = self.parameter.inputPath + '/pdb/1.pdb'
                        ...
                        if self.parameter.iteration == 1:
                            ...
                            if not self.parameter.NVT:
                                if enhanced == 0:
                                    ...
                                    info[0] = '#binvelocities'
                            if enhanced == 1:
                # original info[0] = '#binvelocities'   changed by  Alfredo Sept 2021
                       #         info[0] = '#binvelocities'
                                info[0] = 'binvelocities'
                ...
                if "extendedSystem" in line or "extendedsystem" in line:
                    ...
                    elif self.parameter.namd_conf == True and self.step != 'seek' and self.parameter.iteration == 0:
                        info[0] = 'extendedSystem'
                        info[1] = './sample.xsc'
                ...
                if "temperature" in line and "pressure" not in line:
                    ...
                    if self.step == 'seek':
                        info[0] = 'temperature'
                    if enhanced == 1:
             # original info[0] = 'temperature'  Changed by Alfredo in combination with binvelocities. Sept 2021.
                     #   info[0] = 'temperature'
                        info[0] = '#temperature'
...
#                    line = re.sub(r'[^\w]', ' ', line)
        ...
        for i in self.loop1:
            for j in self.loop2:
                ...
                if self.parameter.milestone_search == 2:
                    if self.parameter.all_grid[str(final_ms[0]) + '_' + str(final_ms[1])][2] == 'c':
                        continue
    ...
    def __post_seek(self, path, final_ms):
        '''
        After seek, this finds all of the milestones that were hit and copies the seek.ms.pdb file to that
            ...
            for ms in self.parameter.MS_list:
                ...
                if os.path.isfile(path):
                    with open(path) as f:
                        for line in f:
                            if "ERROR:" in line:
                                log("Error with sampling on milestone {}. ScMiles terminated".format(ms))
