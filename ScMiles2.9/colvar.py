...
class colvar:
    ...
    def __substitution(self):
        ...
        name = None
        count = 2
        #First we look in our custom.colvar file and look for our customfunction line. The customfunction is where we are doing our substitution
    ...
    def __rmsd_to_anchor(self, anchor, coeff=None, space=0):
        ...
        rmsd = []
        tmp.append('colvar {')
        tmp.append('name rmsd' + str(anchor)) #This is just naming our rmsd after the anchor that we are looking at. So for rmsd to anchor 2, it is rmsd2
        function = self.__pbc_distance(anchor)  #This gives us our function. Not only for pbc, just allows for pbc with l values
        tmp.append('customFunction ' + str(function)) #This is just the line like: customfunction sqrt((psi-175.0)^2 ...
    ...
    def __pbc_distance(self,anchor):
        #This works with voronoi cell, finding our rmsd functions (does not have to be pbc, but can be)
        line = ''
        line += 'sqrt('
        for colvar_index in range(self.colvars_number): #This just loops through each of our colvars.
            name = self.parameter.names[colvar_index] #This is the name of the current colvar we are looking at
            anchor_value = self.parameter.anchors[anchor-1][colvar_index] #This is the anchor values that we are lookinga t for the specific colvar at the anchor we are looking at
            absolute_value = '(' + name + '-' + str(anchor_value) + ')' #Just creating absolute value statement
            if self.parameter.scale_rmsd: #This means that we are scaling the rmsd by a certain value. So if one colvar is weighted more than others.
                scale = str(self.parameter.scale_rmsd[colvar_index]) + '*'
            else:
                scale = '' #If we aren't scaling, we just have an empty string that gets appended
            if name in self.parameter.pbc_names: #If we are using periodic boundary, we go in here.
                line += '('
                name_index = self.parameter.pbc_names.index(name) #Index of our variable in pbc_names (in case user entered them in a different order, or it is not in pbc_names
                l_value = self.parameter.l[name_index]  #This is the l value corresponding to our periodic variable
                l_half = l_value/2
    ...
    def __min_pbc(self, anchor):
        #Here I was testing something, but as of now I am unsure if this works correctly or not so it needs to be fixed or tested AH
        ms_values = []
        rmsd = []
    ...
    def __find_rmsd(self, milestone):
        ...
        #this is if we are using the dist_cut parameter. It decides whether the anchors are closer or further than dist_cut away
        ...
        if dist > self.parameter.dist_cut:
            return False #The distance is greater than our dist_cut, so we return False
        else:
            return True #The distance is closer than our dist_cut so we return True

    def generate(self):   
        '''This is the main function that generates colvars '''
        if self.step == 'seek':  
            outputFrequency = 1
        else: #So free or sample
            outputFrequency = self.parameter.colvarsTrajFrequency
            
        fconf = open(self.config_path, 'w+') #writing to our config path, which is our colvar.txt or colvar_free.txt that is in the ScMiles folder.
        ...
        if self.step == 'sample':
            crd_path = self.parameter.crdPath + '/' + str(self.anchor1) + '_' + str(self.anchor2) #crd/1_2 for anchors 1 and 2
            if not os.path.isfile(crd_path + '/seek.ms.pdb') and not os.path.isfile(crd_path + '/' + str(self.parameter.outputname) + '.coor'): 
                #this means we need to use pdb files to sample, so we either did not use seek, or we are using exact milestoning and have a large restart freq 
                self.pdb_sampling = True

        if self.step != 'sample':
            if self.parameter.milestone_search == 2 or self.parameter.milestone_search == 3: #we are using grid for either seek or sample
                self.__grid_colvars()
            else:
                for i in range(self.parameter.AnchorNum): #This just means voronoi. So cycling through each anchor and finding each appropriate rmsd
                    self.__rmsd_to_anchor(i+1)
        else:
            if self.colvars_number == 1: #One dimensional problem (not grid)
                self.__constraint1D1()
                self.__harmonic1D()
            elif self.parameter.milestone_search == 2 or self.parameter.milestone_search == 3: #grid sampling
                self.__grid_sampling()
            else: #2 or more dimensional voronoi
    ...
    def __grid_corners(self, equations):
        #find corner values, this is not used at the moment, I have it in milestones.py instead
        a1_values = set()
        a2_values = set()
    ...
    def get_final_ms(self, path, anchor=None):
        ...
        if self.parameter.milestone_search == 2:
            ...
            if self.parameter.software == 'namd':
                ...
                if RMSDs.count(value) == 0:
                    ...
                    with open(path + '/stop.colvars.state') as f:
                        for line in f:
                            if 'name' in line:
                                if count in equation:
                                    line = line.split()
                                    end = line[1]
                                    if 'low' in end or 'high' in end or 'corner' in end:
                                        return -1, [0,0]
                                    possible.append(end)
                                count += 1
                        print(possible)
                        if len(possible) == 1:
                            final_ms = get_anchors(possible[0])
                        else:
                            for i in range(len(possible)):
                                if self.parameter.all_grid[possible[i]][2] == 'c':
                                    continue
                                final_ms = get_anchors(possible[i])
                                break
                    '''
                    #print(path)
                    possible = []
            ...
            if self.parameter.pbc and self.parameter.software == 'namd':
            ...
            elif self.parameter.pbc and self.parameter.software == 'gromacs':
                if anchor:
                    if final_ms[0] == self.parameter.AnchorNum and anchor == 1:
                        final_ms[1] = 1
                    elif final_ms[0] == 1 and anchor == self.parameter.AnchorNum:
                        final_ms[1] = self.parameter.AnchorNum
                    else:
                        # use the second min value for previous cell #
                        final_ms[1] = RMSDs.index(sorted(RMSDs)[1]) + 1
                else:
                    starting_anchors = get_anchors(path.split('/')[-3])
                    pbc1 = [final_ms[0], starting_anchors[1]]
                    pbc2 = [starting_anchors[0], final_ms[0]]
                    if pbc1 == self.parameter.pbc:
                        final_ms[1] = starting_anchors[1]
                    elif pbc2 == self.parameter.pbc:
                        final_ms[1] = starting_anchors[0]
                    else:
                        final_ms[1] = RMSDs.index(sorted(RMSDs)[1]) + 1 
            else:
                # use the second min value for previous cell #
...
if __name__ == '__main__':
    ...
    new.milestone_search = 2
    new.iteration = 2
    print(colvar(new, step='free', anchor1=55,anchor2=56).get_final_ms(new.crdPath + '/55_56/2/1'))
