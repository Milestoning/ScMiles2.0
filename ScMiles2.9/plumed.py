...
class plumed:
    ...
    def __sampling_voronoi(self):
        ...
        if self.parameter.pbc:
            if [self.anchor1, self.anchor2] == self.parameter.pbc:
                full_function.append("FUNC=x1")
                full_function.append("PERIODIC=-pi,pi")
            else:
        ...
        if self.parameter.pbc:
            if self.parameter.pbc[0] == self.anchor1 and self.parameter.pbc[1] == self.anchor2:
                full_function.append("AT=3.14159")
            else:
        ...
        full_function.append("PRINT STRIDE=100 ARG=" + ",".join(names) + ",neighbor," + ",".join(label) + " FILE=" + self.parameter.outputname + '.colvars.traj')
        fconf = open(self.config_path, 'a')
        for i in full_function:
            print(i,file=fconf)
        fconf.close()
