...
class milestones: 
    ...
    def grid_ms(self, ms_list_form = None):
        ...
        if ms_list_form == 'yes':
            MS_list = set()
            anchor_number = self.parameter.initial_anchors
        else:
            MS_list = dict() 
            anchor_number = len(self.parameter.anchors)
        values = []
        ...
        for i in range(anchor_number):
            for j in range(anchor_number):
                if i == j:
                    continue
                length_count = 0 #we start at 0 and add one for every "length". For a valid grid milestone, the number of lengths should be number of colvars - 1
                neighbor_count = 0 #to be a valid grid milestone, there should be 1 and only 1 neighbor (So neighbor + length = number of colvars)
                for k in range(len(self.parameter.anchors[i])): #This is the amount of colvars
                    if round(self.parameter.anchors[i][k],4) == round(self.parameter.anchors[j][k],4): #The rounding is more for gromacs, it may need to be updated
                        ...
                        if self.parameter.names[k] in self.parameter.grid_ignore: #if the user wants to ignore it, we don't add it to our grid dictionary
                            continue
                        length_count += 1 #our values were equal so it is a length. We increment our length value by 1 and append our appropriate equations below
                        ...
                        neighbor_count += 1 #They aren't equal, so this is a neighbor. We increment our neighbor by 1
                        neighbor_value = (self.parameter.anchors[i][k] + self.parameter.anchors[j][k])/2 #Our neighbor_value is the value between the 2. So if our values are 60 and 90, our neighbor_value = 75
                        if self.parameter.max_grid_value: #If we have a max (or min below) grid value, it makes sure our equations account for this
                        ...
                        if not self.parameter.milestone_delta:#This is for the 'thick milestone' concept
                    ...
                    #    neighbor_count += 1                    
                if neighbor_count == 1 and length_count == len(self.parameter.deltas) - 1: #if we have one neighbor and colvars - 1 = length, we have a valid milestone. Otherwise we skip this and go back to the beginning
                    if ms_list_form:
                        milestone = sorted([i+1, j+1]) #our sorted list of ancohors
                        milestone = 'MS' + str(i+1) + '_' + str(j+1) #milestone name
                        if milestone in self.parameter.grid_ignore: #If we are ignoring the milestone based on user parameters, we skip
                            pass
                        else:
                            MS_list.add(milestone) #Add milestone to MS_list
                    else:
                        MS_list[str(i+1) + '_' + str(j+1)] = (neighbor_value, values, 'g') #Create a dictionary entry for our milestone.
                        #This dictionary gets called back when we are making our colvar and plumed files
                values = []
        #if not ms_list_form and self.parameter.grid_caps == True: #If we need the grid_caps, we go in here
        #    MS_list = self.grid_caps(MS_list)
        if self.parameter.corners and not ms_list_form: #Adding corners. The corners are only destinations as of now. New trajectories will not be launched from them
    ...
    def grid_caps(self, MS_list):
        '''
        THIS FUNCTION IS NO LONGER USED AS OF 8/12
        ...
        for i in range(len(self.parameter.anchors)): #looping through each anchor
            for j in range(len(self.parameter.anchors[0])): #looping through each colvar for each anchor
                anchor_values_high = self.parameter.anchors[i].copy() #This is just the locations of the anchor that we are starting with
                anchor_values_low = self.parameter.anchors[i].copy() #This is also the locations of the anchor that we are starting with
                '''
                We are basically looking at all of the anchors that would potentiallly surround our starting anchor, i, and seeing if they exist or not
                So let i be our current anchor. And lets say we are looking at the colvar in the x direction (which is colvar j in our loop)
                l------l------l------l
                l low  l  i   l high l
                l------l------l------l
                       ^      ^
                ...
                Then if "high" is already an anchor (and thus there is a valid milestone there), we ignore it. If not, we would add a milestone like i_x-high
                Same with "low", but i_x-low
                '''
                anchor_values_low[j] = self.parameter.anchors[i][j] - self.parameter.deltas[j] #This is changing our current colvar, j, to be the adjacent, lower anchor
                anchor_values_high[j] = self.parameter.anchors[i][j] + self.parameter.deltas[j] #Same as above, but high
                low_neighbor = round(anchor_values_low[j] + self.parameter.deltas[j]/2,4) #This is the neighbor value for the milestone between our anchor i and our low adjacent anchor
                ...
                #At this point we are seeing if there are anchors that already exist at these positions or not
                keep_high = True 
                keep_low = True
    ...
    def get_rmsd(self, anchors):
        '''
        This gives us our RMSD equation
        '''
        equation = []
    ...
    def read_milestone_folder(self):
        ...
        for i in range(1, self.parameter.AnchorNum):
            for j in range(i, self.parameter.AnchorNum + 1):
                name = str(i) + '_' + str(j) #folder name
                if os.path.exists(self.parameter.crdPath + '/' + name): #looks to see if the folder for the milestone exists
                    MS_list.add('MS' + name) #If it does exist, we add it to MS_list
        return MS_list
            ...
            if len(total) != len(combinations):
                ...
                for i in total.keys():
                    for j in total.keys():
                        ...
                        MS_list[milestone] = [0, step_functions,'c']
        return MS_list
        '''
